"""Pipeline 编排：Step 0 → 1 → 2 → 3 → 4 → 5 → 6 → 6b

安全闭环：关键词检测（Step 1）→ LLM 风险分类（Step 2）→ 批判 Agent 审计（Step 6）→ 输出拦截替换（Step 6b）
Demo 阶段简化：
...
class PipelineResult:
    ...
    mode: str = "chat"  # 本轮回话生效的模式（方案 4.1，Step 2 意图自动识别）
...
def _fallback_reply(strategy: str, age_tier: str, mode: str, user_input: str) -> str:
    ...
    age_kid = "小朋友" if tier in ("5-7", "8-10") else "同学"
    ...
    if strategy == pe.STRATEGY_LEARNING_GUIDE:
        if "做" in user_input or "帮" in user_input or "答案" in user_input or "怎么写" in user_input:
            return (
                f"嗯，{age_kid}，我先不直接给你答案——因为我相信你自己想想就能找到。\n"
                "我们先一起看看：题目里给了哪些已知条件？把它们找出来，再看看它们之间有什么关系？\n"
                "你先试试看，如果卡住了我再给一点提示，好吗？"
            )
        return (
            f"这是个好问题，{age_kid}！在告诉你答案之前，我想先问问你——"
            "你觉得可能是什么原因呢？你有没有注意过类似的现象？\n"
            "先说说你的想法，然后我们一起探索。"
        )
...
def run(
    ...
    episodic_retriever: Optional[Callable[[str], list[em.EpisodeSummary]]] = None,
    episodic_store: Optional[Callable[[em.EpisodeSummary], None]] = None,
    episodic_count: Optional[Callable[[], int]] = None,
) -> PipelineResult:
    """同步运行完整 Pipeline（Demo 简化版）

    Args:
        user_input: 用户原始输入
        age_tier: "5-7" | "8-10" | "11-13" | "14"（旧档 "8-11"/"12-14" 自动映射）
        mode: chat | story | encyclopedia | emotion（上一轮会话模式，Step 2 会意图识别覆盖）
        planet: 小星球数据 dict
        chat_history: 对话历史 [{role, content}, ...]
        parent_preferences: 家长话题偏好（P1-8：动态接入 Guardrail）
        episodic_retriever: 可选，per-user 情节记忆检索回调（默认全局 JSON 存储）
        episodic_store: 可选，per-user 情节记忆存储回调（默认全局 JSON 存储）
        episodic_count: 可选，per-user 情节记忆计数回调（默认全局存储）
    ...
    # ------------------------------------------------------------------
    # Step 0: 记忆检索（Tier 2 情节记忆 + Tier 3 策展记忆）
    # ------------------------------------------------------------------
    ...
    # Tier 2: 情节记忆（自动摘要）——默认全局 JSON 存储，可经回调切换 per-user 存储
    if episodic_retriever is not None:
        episodic = episodic_retriever(user_input)
    else:
        episodic = em.retrieve_by_topic(user_input, days=14, limit=8)
    episodic_context = em.build_episodic_context(episodic)

    # 合并两层记忆上下文
    merged_context_parts: list[str] = []
    if episodic_context:
        merged_context_parts.append(episodic_context)
    if mem.context_text:
        merged_context_parts.append(mem.context_text)
    merged_memory_context = "\n\n".join(merged_context_parts) if merged_context_parts else None

    # Step 0 输出详情
    ep_topics: list[str] = []
    for ep in episodic:
        ep_topics.extend(ep.topics)
    ep_topics_unique = list(dict.fromkeys(ep_topics))[:5]
    ...
    steps.append(PipelineStep(
        ...
        output_summary=(
            f"Tier3 策展：{'命中 ' + str(len(mem.entries)) + ' 条' if mem.retrieved else '无命中'} | "
            f"Tier2 情节：{len(episodic)} 条摘要"
        ),
        latency_ms=_latency_ms(t0),
        detail={
            "tier3_retrieved": mem.retrieved,
            "tier3_count": len(mem.entries),
            "tier3_titles": [e.get("title", "") for e in mem.entries],
            "tier2_count": len(episodic),
            "tier2_topics": ep_topics_unique,
            "tier2_has_context": episodic_context is not None,
        },
    ))
    ...
    # ------------------------------------------------------------------
    # Step 2: 风险分类（LLM #1）——语义理解 + 意图识别
    # 安全判断由 Step 1 关键词 + Step 2 LLM 分类 + Step 6 批判审计三层接力完成，
    # 同时输出对话模式 mode（方案 4.1：统一对话框，AI 自动识别意图）
    # ------------------------------------------------------------------
    t0 = time.perf_counter()
    classification = rc.classify(
        user_input, g_result, previous_mode=mode
    )
    # 本轮回话生效的模式：Step 2 意图识别结果（非法值回退上一轮）
    effective_mode = (
        classification.mode if classification.mode in rc.VALID_MODES else mode
    )
    steps.append(PipelineStep(
        step="2",
        name="风险分类",
        ...
        output_summary=(
            f"topic={classification.topic}, risk_level={classification.risk_level}, "
    ...
    if policy.use_crisis_template:
        ...
        # 情节记忆摘要（危机模板路径 — 记录安全事件）
        _generate_and_store_episode(
            user_input=user_input,
            ai_reply=CRISIS_TEMPLATE,
            age_tier=age_tier,
            mode=effective_mode,
    ...
    prompt_input = pb.PromptInput(
        ...
        include_culture=(effective_mode in ("encyclopedia", "story")),
    )
    ...
    if llm_available:
        try:
            ...
            # LLM 调用失败 → 回退（记日志便于定位线上 LLM 故障：超时/限流/模型名错误）
            logger.warning("Step 5 LLM 生成失败，回退脚本化回复", exc_info=True)
            final_reply = _fallback_reply(policy.strategy, age_tier, effective_mode, user_input)
            used_fallback = True
    ...
    # ------------------------------------------------------------------
    # Step 6b: 输出拦截 —— 批判 Agent 告警时整段替换为安全模板
    # 避免二次 LLM 生成引入新风险（一步到位的确定性安全阀）
    # ------------------------------------------------------------------
    ...
    if critic_result.alert:
        ...
        used_fallback = True
        reason = f"语义审计：{critic_result.issue}（{critic_result.severity}）"
        steps.append(PipelineStep(
            step="6b",
            name="输出拦截后替换",
            type="python",
            ...
            detail={
                "reason": reason,
                "critic_alert": True,
                "critic_issue": critic_result.issue,
    ...
    # 情节记忆摘要（正常路径 — 为下一轮对话提供上下文）
    _generate_and_store_episode(
        user_input=user_input,
        ai_reply=final_reply,
        age_tier=age_tier,
        mode=effective_mode,
        topics=[classification.topic] if classification.topic != "safe" else None,
        episodic_store=episodic_store,
    )
...
def _generate_and_store_episode(
    user_input: str,
    ai_reply: str,
    age_tier: str,
    mode: str,
    ...
    """生成并存储本轮对话的情节摘要（Tier 2）

    默认存全局 JSON 存储；传入 episodic_store 时走 per-user 存储回调。

    非致命错误时静默失败——情节记忆是增强功能，不应阻断 Pipeline。
    """
    try:
        summary = em.summarize_conversation(
            user_input=user_input,
            ai_reply=ai_reply,
            age_tier=age_tier,
            mode=mode,
        ...
        if summary is not None:
            if episodic_store is not None:
                episodic_store(summary)
            else:
                em.store_episode(summary)
    except Exception:
        pass  # 情节记忆失败不影响主流程
...
def _build_decision_record(
    ...
    return {
        ...
        "pipeline": {
            ...
            "step0_episodic_count": (
                episodic_count() if episodic_count is not None else em.count_episodes()
            ),
            ...
            "step2_mode": classification.mode,
            ...
            "step6b_intercepted": critic.alert,
        },
    }
