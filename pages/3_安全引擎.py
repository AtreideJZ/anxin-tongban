...
st.caption("技术监控页面。每条消息从输入到回复经过多层安全 Pipeline，下方展示实际运行链路。")
...
# 输出拦截分支图例用 risk_3 的半透明底色（由色板派生，等同原 rgba(217,83,79,0.5)）
_r3_hex = styles.COLORS["risk_3"].lstrip("#")
_intercept_legend_bg = (
    f"rgba({int(_r3_hex[0:2], 16)},{int(_r3_hex[2:4], 16)},{int(_r3_hex[4:6], 16)},0.5)"
)
legend_col1, legend_col2, legend_col3, legend_col4 = st.columns(4)
with legend_col1:
    st.markdown(
        f'<div class="meta"><span style="display:inline-block;width:10px;height:10px;'
        f'background:{styles.COLORS["blue"]};border-radius:2px;vertical-align:middle;"></span> LLM 节点</div>',
        unsafe_allow_html=True,
    )
with legend_col2:
    st.markdown(
...
with legend_col4:
    st.markdown(
        f'<div class="meta"><span style="display:inline-block;width:10px;height:10px;'
        f'background:{_intercept_legend_bg};border-radius:2px;vertical-align:middle;"></span>'
...
# 步骤类型 pill 配色（前景/背景与 styles.py 中 pipeline-node 节点配色一致）
_STEP_TYPE_PILLS = {
    "llm": ("LLM", styles.COLORS["blue"], "rgba(59,130,246,0.1)"),
    "python": ("Python", "#5b8b4d", "rgba(123,183,110,0.12)"),
    "crisis": ("危机", styles.COLORS["risk_3"], "rgba(217,83,79,0.1)"),
    "intercept": ("拦截", styles.COLORS["risk_3"], "rgba(217,83,79,0.1)"),
}
...
for step in latest.steps:
    ...
    type_label, pill_fg, pill_bg = _STEP_TYPE_PILLS[type_class]
    st.markdown(
        f"""
        <div class="pipeline-step {type_class}">
            <div style="display:flex; justify-content:space-between; align-items:baseline;">
                <div>
                    ...
                    {styles.pill(type_label, pill_fg, pill_bg)}
                </div>
    ...
    if step.step == "1":
        ...
        if hits:
            for h in hits:
                ...
                if cat == "parent_forbidden_topic":
                    color_hint = f"color: {styles.risk_color(3)};"
                elif cat == "parent_limited_topic":
                    color_hint = f"color: {styles.risk_color(2)};"
                st.markdown(
        ...
        if d.get("alert"):
            st.markdown(
                f'<div class="meta" style="margin-left: 20px; color: {styles.risk_color(3)};">'
...
if getattr(latest, "critic_intercepted", False) and latest.original_intercepted_reply:
    with st.expander("🔴 查看被拦截的原始回复（未展示给孩子）", expanded=False):
        st.markdown(
            ...
            f'border-left: 4px solid {styles.risk_color(3)};">{latest.original_intercepted_reply}</div>',
            unsafe_allow_html=True,
        )
    st.warning(
...
if not all_runs:
    ...
    for i, r in enumerate(all_runs):
        risk_dot_color = styles.risk_color(r.risk_level)
        col_x, col_y, col_z = st.columns([3, 2, 2])
        with col_x:
            st.markdown(
                f'<div><b>#{len(all_runs) - i}</b> · '
                f'<span style="color:{risk_dot_color};">●</span> '
...
with nav3:
    if st.button("👨‍👩‍👧 家长端", use_container_width=True):
