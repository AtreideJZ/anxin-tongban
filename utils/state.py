...
_EPISODIC_FILE = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data", "episodic_memory.json")
...
def init_state() -> None:
    ...
    # 初始化情节记忆存储（Tier 2）
    # 部署环境下也初始化（JSON 在内存中，不持久化到磁盘即可保持隔离）
    _init_episodic_store()
...
def _init_episodic_store() -> None:
    """初始化情节记忆存储路径

    本地开发：JSON 文件持久化（data/episodic_memory.json）
    部署环境：内存存储（session 级别，容器关闭即清空）
    """
    from core import episodic_memory as em
    store_path = _EPISODIC_FILE if _PERSISTENCE_ENABLED else ":memory:"
    em.init_store(store_path)
